...
$sql=$wpdb->get_results("SELECT * FROM teacher_registration WHERE email='".$user_email."'");
...
if($name){
      move_uploaded_file($tmp_name,$upload_dir['basedir'].'/documents/'.$name);
	        global $wpdb;
            $table = 'file';
            $data = array(
				'topic'=>$doc_name,
                'name'    => $name,
				'path'   => $upload_dir['basedir'].'/documents/'.$name,
				'date'   => $date,
				'user'   => $user_fname,
				'course'=> $a
  ...
  echo 'setTimeout(function () { swal("Uploaded successful!","success");';
  echo '}, 100);</script>';
...
<body>
		...
		<div class="page-container">
			...
			<h2><?php echo $title; ?></h2>
			  ...
			  <br /> 
				<form name="file" enctype="multipart/form-data" method="post"  class="main"> 
			<input type="text" name="doc_name" placeholder="Topics Name" />
			<input type="file" name="myfile" />
			[*File format must be pdf or mp4]</br>
